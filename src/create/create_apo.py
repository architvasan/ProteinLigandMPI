...
class CreationSteps():
    def __init__(self, pdb_id, ambertools_env, leapin, leapin_ion, struct_dir):
        self.pdb_id = pdb_id
    ...
    def step2(self, inp_pdb2, out_pdb2):
        ...
        os.system(f'/soft/applications/conda/2024-04-29/mconda3/condabin/conda run -p {self.ambertools_env} pdb4amber -i {self.struct_dir}/{inp_pdb2} -o {self.struct_dir}/{out_pdb2}')
    ...
    def step3(self):
        ...
        if True:
            os.system(f'/soft/applications/conda/2024-04-29/mconda3/condabin/conda run -p {self.ambertools_env} antechamber -j 5 -at sybyl -dr no -i {self.struct_dir}/ligid.H.parsed.pdb -fi pdb -o {self.struct_dir}/ligid.mol2 -fo mol2 -c bcc -nc 0 -rn LIG -at gaff2')

        os.system(f'/soft/applications/conda/2024-04-29/mconda3/condabin/conda run -p {self.ambertools_env} parmchk2 -i {self.struct_dir}/ligid.mol2 -f mol2 -o {self.struct_dir}/ligid.frcmod -s gaff2')
    ...
    def step4(self):
        ...
        with open(self.leapin, 'a') as leapfile:
            ...
            leapfile.write(f'logfile {self.struct_dir}/logfile_solvate.log\n \
            ...
            leapfile.write('# Solvate\n \
                                ...
                                quit')
        os.system(f'/soft/applications/conda/2024-04-29/mconda3/condabin/conda run -p {self.ambertools_env} tleap -f {self.leapin}')
        os.system(f'rm {self.leapin}')

    def step5(self):
        n_ions = (obtain_nions(f'{self.struct_dir}/logfile_solvate.log', 0.15))
        print(n_ions)
        os.system(f'grep -vwE "CONECT" {self.struct_dir}/protein4amber.pdb > {self.struct_dir}/protein4amber1.pdb')
        os.system(f'mv {self.struct_dir}/protein4amber1.pdb {self.struct_dir}/protein4amber.pdb')
        with open(self.leapin_ion, 'w') as leapfile:
            leapfile.write('')
        with open(self.leapin_ion, 'a') as leapfile:
            leapfile.write(f'logfile {self.struct_dir}/logfile_ionize.log\n \
                    # Load protein, ligand and water molecules\n \
                    source leaprc.water.opc\n\
                    source leaprc.protein.ff19SB\n\
                    protein1 = loadPDB {self.struct_dir}/protein4amber.pdb\n')

            leapfile.write('system1 = combine {protein1} \n \
                    check system1\n')
            ...
            leapfile.write(f'# Solvate\n \
                    ...
                    quit')
        os.system(f'/soft/applications/conda/2024-04-29/mconda3/condabin/conda run -p {self.ambertools_env} tleap -f {self.leapin_ion}')
        #os.system(f'rm {self.leapin_ion}')
        
        #system = loadPDB struct/system.dry.pdb\n \

def run_steps(pdbid, structdir, ambertools_env, leapin, leapion):


    Creation = CreationSteps(pdbid,  ambertools_env, leapin, leapion, structdir)
    '''
    Obaining PDB
    '''
    print("Obaining PDB")
...
if __name__ == "__main__":
    ...
    try:
        os.mkdir(args.structdir)
    except:
        pass

    run_steps(args.pdbid, args.structdir, args.ambertools, args.leapin, args.leapion)
