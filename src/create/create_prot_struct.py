...
class CreationSteps():
    ...
    def step0(self):
        ...
        os.system(f'grep -v -e "{self.ligid}" -e "GWS" -e "B" -e "DMS" -e "CONECT" -e "HOH" {pdb_filename}.pdb > struct/protein_pre.pdb')
        import MDAnalysis as mda
        u = mda.Universe("struct/protein_pre.pdb")
    ...
    def step3(self):
        ...
        if True:
            os.system(f'conda run -p {self.ambertools_env} antechamber -j 5 -at sybyl -dr no -i struct/ligid.H.parsed.pdb -fi pdb -o struct/ligid.mol2 -fo mol2 -c bcc -nc 0 -rn LIG -at gaff2')

        os.system(f'conda run -p {self.ambertools_env} parmchk2 -i struct/ligid.mol2 -f mol2 -o struct/ligid.frcmod -s gaff2')
